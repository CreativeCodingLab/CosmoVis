...
### CosmoVis Data Preprocessing - make PartType files for halos in the EAGLE 100 Mpc simulation
...
halocat_100_lowmh = ascii.read('static/data/halo_cats/halos_'+str(EAGLE100)+'_lowm'+'.txt')
...
field_list_todo = [("PartType0", "xray_luminosity_0.1_2_keV")]
...
# for i in range(len(halocat_100_lowmh)):

# for i in range(223,len(halocat_100_midmh)):

# for i in range(len(halocat_100_midmh)):

for i in range(29):

# for i in range(2140,2141):
# for i in range(1054,1055):
    ...
    logger.info("=============In halo {} of {}, ID {}, mh = {}x10^13 \n ============".format(i+1,len(halocat_100_highmh),thishalo_id,round(float(thishalo_mh)/1e13,2)))
    ...
    # thishalo_id, thishalo_mh, thishalo_rh = halocat_100_lowmh['haloID'][i], halocat_100_lowmh['mh'][i], (halocat_100_lowmh['rh'][i])/1000
    
    # thishalo_x, thishalo_y, thishalo_z = halocat_100_lowmh['halo_x'][i], halocat_100_lowmh['halo_y'][i], halocat_100_lowmh['halo_z'][i]

    # #### for spin:

    # thishalo_gx, thishalo_gy, thishalo_gz = halocat_100_lowmh['gal_x'][i], halocat_100_lowmh['gal_y'][i], halocat_100_lowmh['gal_z'][i]
    # thishalo_galcenter = [thishalo_gx, thishalo_gy, thishalo_gz]

    # thishalo_sx, thishalo_sy, thishalo_sz = halocat_100_lowmh['spin_x'][i], halocat_100_lowmh['spin_y'][i], halocat_100_lowmh['spin_z'][i]
    # thishalo_galspin = [thishalo_sx, thishalo_sy, thishalo_sz]   

    # if float(thishalo_mh) >= 1.0e11:
    #     logger.info("=============In halo {} of {}, ID {}, mh = {}x10^11 \n ============".format(i+1,len(halocat_100_lowmh),thishalo_id,round(float(thishalo_mh)/1e11,2)))

    # elif float(thishalo_mh) < 1.0e10:
    #     logger.info("=============In halo {} of {}, ID {}, mh = {}x10^9 \n ============".format(i+1,len(halocat_100_lowmh),thishalo_id,round(float(thishalo_mh)/1e9,2)))

    # else:
    #     logger.info("=============In halo {} of {}, ID {}, mh = {}x10^10 \n ============".format(i+1,len(halocat_100_lowmh),thishalo_id,round(float(thishalo_mh)/1e10,2)))
    ...
    if float(thishalo_mh) >= 1.0e13:
        ...
        starpercent = 33 #export a fraction of all the stars

    elif 1.0e13 > float(thishalo_mh) >= 4.0e12:
        rhMult = 3  #multiplier of Rh for box half width
        starpercent = 75 #export a fraction of all the stars

    elif 4.0e12 > float(thishalo_mh) >= 1.0e12:
        rhMult = 3  #multiplier of Rh for box half width
        starpercent = 100 #export a fraction of all the stars
        resolution_list = [64,128]
    
    elif float(thishalo_mh) < 1.0e12:
        rhMult = 3  #multiplier of Rh for box half width
        starpercent = 100 #export a fraction of all the stars
        resolution_list = [64,128]
    ...
    extractFeatures(ds,EAGLE100,'EAGLE',resolution_list,field_list_todo,left_edge,right_edge,thishalo_id,thishalo_galcenter,thishalo_galspin)
    
    # exportSimMetadata(EAGLE100,left_edge,right_edge,thishalo_id,thishalo_galcenter,thishalo_galspin)

    logger.info("=============Zoom-in created, box dims:{}--{} \n ============".format(left_edge,right_edge))
