...
with open('static/data/gas/gasShuffle.json') as L_gas:
    L_gas = json.load(L_gas)

with open('static/data/dm/dmShuffle.json') as L_dm:
    L_dm = json.load(L_dm)

with open('static/data/stars/starsShuffle.json') as L_stars:
    L_stars = json.load(L_stars)

with open('static/data/bh/bhShuffle.json') as L_bh:
    L_bh = json.load(L_bh)
...
# with open('static/data/gas/gasShuffle.json', 'w') as file:
#     json.dump(L, file)

# L_dm = np.arange(len(np.array(ad['PartType1', 'Coordinates'])))
# random.shuffle(L_dm)
# L = L_dm.tolist()

# with open('static/data/dm/dmShuffle.json', 'w') as file:
#     json.dump(L, file)

# L_stars = np.arange(len(np.array(ad['PartType4', 'Coordinates'])))
# random.shuffle(L_stars)
# L = L_stars.tolist()

# with open('static/data/stars/starsShuffle.json', 'w') as file:
#     json.dump(L, file)

# L_bh = np.arange(len(np.array(ad['PartType5', 'Coordinates'])))
# random.shuffle(L_bh)
# L = L_bh.tolist()

# with open('static/data/bh/bhShuffle.json', 'w') as file:
#     json.dump(L, file)
...
def createLookup():
    ...
    # ids = list(ids)
    # random.seed(4)
    # pos_sample = random.sample(positions, 100)
    # pos_sample
    print('creating lookups')
    def subset(type,L,positions,k):
        for i in range(k):
            shuffle = []
            for j in range(N):
                try:
                    index = L[(i*N)+j]
                    shuffle.append(positions[index])
                except:
                    continue
            with open('static/data/' + type + '/' + type + 'Lookup-sub' + str(k) + '.' + str(i) + '.json', 'w') as file:
                json.dump(shuffle, file)
    ...
    # # L_gas = np.arange(len(np.array(ad['PartType0', 'Coordinates'])))
    # # random.shuffle(L_gas)
    # subset('gas',L_gas,positions,1)

    positions = np.array(ad['PartType1', 'Coordinates'])
    positions = np.around(positions,6)
    positions = positions.tolist()
    # L_dm = np.arange(len(np.array(ad['PartType1', 'Coordinates'])))
    # random.shuffle(L_dm)
    subset('dm',L_dm,positions,16)

    positions = np.array(ad['PartType4', 'Coordinates'])
    positions = np.around(positions,6)
    positions = positions.tolist()
    # L_stars = np.arange(len(np.array(ad['PartType4', 'Coordinates'])))
    # random.shuffle(L_stars)
    subset('stars',L_stars,positions,16)

    positions = np.array(ad['PartType5', 'Coordinates'])
    positions = np.around(positions,6)
    positions = positions.tolist()
    # L_bh = np.arange(len(np.array(ad['PartType5', 'Coordinates'])))
    # random.shuffle(L_bh)
    subset('bh',L_bh,positions,16)
    ...
    #     for j in range(N):
    #         gas_shuffle[j] = positions[L_gas[i*j]]

    #     with open('static/data/gas/gasLookup-sub'+i+'.json', 'w') as file:
    #         json.dump(gas_shuffle, file)
    # # lookup = tuple(zip(ids, positions))

    # with open('static/data/gas/gasLookup-min-0376.json', 'w') as file:
