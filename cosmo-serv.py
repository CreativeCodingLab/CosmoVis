...
socketio = SocketIO(app,cors_allowed_origins="https://cosmovis-scidev.nrp-nautilus.io", async_mode=async_mode,async_handlers=True,upgradeTimeout=240000)#,logger=True, engineio_logger=True)
# thread = None
# thread_lock = Lock()
...
def handle_skewer_simple_ray(simID,idx,start,end):
    ...
    ###  add a zoom-in variable: handle_skewer_simple_ray(simID,idx,start,end,zoom=false,halos=None)
    ...
    # ad = ds.r[(0, "kpc"):(5000, "kpc"), (7500, "kpc"):(12500, "kpc"), (0, "kpc"):(5000, "kpc")] 
    # from the simmetadata file (halos)
    ...
    # print('width ',sim_width,ds.domain_right_edge[0],ds.domain_left_edge[0])

    # print(ds.domain_right_edge)

    sim_width = np.float(ad.right_edge[0].in_units('Mpc') - ad.left_edge[0].in_units('Mpc'))
    # print('width ',sim_width,ad.right_edge[0],ad.left_edge[0])
    ...
    ray_start = ds.arr(ray_start/sim_width * np.float(ad.right_edge[0]), 'code_length') #list(np.float_(start))
    ray_end   = ds.arr(ray_end/sim_width * np.float(ad.right_edge[0]), 'code_length')   #list(np.float_(end))

    print('start, end',ray_start,ray_end)
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='0.0.0.0', port=1000, debug=False)
