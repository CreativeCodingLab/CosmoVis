...
THREE.VolumeRenderShader = {
	...
	uniforms: {
		...
		"u_glyphVisibility": { value: false },
		"u_glyphWidth": { value: new THREE.Vector2(1, 1) },
		"u_glyphOffset": { value: new THREE.Vector2(0, 0) },
		"u_glyphDiffuse": { value: null },
		"u_glyphDepth": { value: null },
	...
	fragmentShader: [
		...
		"		uniform bool u_glyphVisibility;",
		"		uniform sampler2D u_glyphDiffuse;",
		" 		uniform sampler2D u_glyphDepth;",
		"		uniform vec2 u_glyphWidth;",
		...
		`
				bool withinBoundaries(vec3 pos){
					// https://discourse.threejs.org/t/clipping-a-volume-in-three-js-giving-black-area-instead-of-inner-material/7572
					if (pos.x < u_xyzMin.x*u_size.x ||
						pos.y < u_xyzMin.y*u_size.y ||
						pos.z < u_xyzMin.z*u_size.z ||
						pos.x > u_xyzMax.x*u_size.x ||
						pos.y > u_xyzMax.y*u_size.y ||
						pos.z > u_xyzMax.z*u_size.z) {
							return false;
						}
					else{
						return true;
				...
				fragCoordZ = texture2D(u_glyphDepth, gl_FragCoord.xy/vec2(u_screenWidth,u_screenHeight)).x;
				viewZ = perspectiveDepthToViewZ(fragCoordZ,u_cameraNear,u_cameraFar);
				float glyphDepth = perspectiveDepthToViewZ( viewZ, u_cameraNear, u_cameraFar );
				...
				if(u_skewerVisibility == true) {
					...
					path_L += u_skewerBrightness * transmittance * skewerCol; ((0.4*path_L) + (0.6*(skewerCol))); //50.0 * sqrt(transmittance) * skewerCol; //
					
					// if(skewerCol.r != 0.0)
					// else path_L += 100.0 * transmittance * skewerCol;
				}
				// bool u_glyphVisibility = true;
				if(u_glyphVisibility == true) {
					vec3 glyphCol = texture2D(u_glyphDiffuse,vec2((gl_FragCoord.x * u_glyphWidth.x) / u_screenWidth + u_glyphOffset.x, (gl_FragCoord.y * u_glyphWidth.y) / u_screenHeight + u_glyphOffset.y)).rgb;
					//glyphCol.xy = gl_FragCoord.xy/vec2(u_glyphWidth.x / u_screenWidth + u_glyphOffset.x, u_glyphWidth.y / u_screenHeight + u_glyphOffset.y);
					//glyphCol.xy = vec2((gl_FragCoord.x * u_glyphWidth.x) / u_screenWidth + u_glyphOffset.x, (gl_FragCoord.y * u_glyphWidth.y) / u_screenHeight + u_glyphOffset.y);

					// glyphCol = vec3(0.0,1.0,0.0);
					path_L += transmittance * glyphCol; // 1000.0 *
				}
