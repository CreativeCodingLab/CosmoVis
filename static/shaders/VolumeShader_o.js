...
THREE.VolumeRenderShader1 = {
	...
	fragmentShader: [
		...
		"				int nsteps = int(-distance / u_stepSize );",
		"				if ( nsteps < 1 )",
		"						discard;",
		...
		"				/* Sample float value from a 3D texture. Assumes intensity data. */",
		"				float density = texture(u_density, texcoords.xyz).r;",
		...
		"				uint random_uint = uint((m_z << 16U) + m_w);",
		"				return float(random_uint)/float(0xFFFFFFFFU);",
		"		}",


		"		vec3 apply_dvr_colormap(float val, bvec2 clip, vec2 clim, sampler2D cm_texture, float density, float dist, vec3 texcoords) {",
		"				float a;",
		"				vec4 tex;",
		...
		// "								c_gas = apply_dvr_colormap(gasVal,u_gasClip,u_gasClim,u_cmGasData,density,0.0,loc);",
		// "								c_dm = apply_dvr_colormap(dmVal,u_dmClip,u_dmClim,u_cmDMData,density,0.0,loc);",
		// // "								c_gas.rgb *= c_gas.a;",
		// // "								c_dm.rgb *= c_dm.a;",
		// "								if(c_dm.a > c_gas.a){",
		// "									c = c_dm;",
		// // "									c.a = 2.0*c_dm.a;",
		// "								}",
		// "								else if(c_dm.a < c_gas.a){",
		// "									c = c_gas;",
		// "								}",
		// "								else{",
		// "									c = mix(c_gas,c_dm,0.5);",
		// "								}",
		// // "								}",
		// // "								else if(c_dm.a == 0.0 && c_gas.a != 0.0){",
		// // "									c = c_gas;",
		// // "								}",
		// // "								else if(c_dm.a != 0.0 && c_gas.a == 0.0){",
		// // "									c = c_dm;",
		...
		// // "								c.a*=2.0;",
		// "								c.a = (c_gas.a+c_dm.a);",
		// // "								float c_r = c_gas.r*c.a + c_dm.r*(1.0-c.a);",
		// // "								float c_g = c_gas.g*c.a + c_dm.g*(1.0-c.a);",
		// // "								float c_b = c_gas.b*c.a + c_dm.b*(1.0-c.a);",
		// // "								float c_a = c_gas.a + c_dm.a*(1.0-c.a);",
		// // "								c = vec4(c_r,c_g,c_b,c_a);",
		// // "								c.rgb *= c.a;",
		// "							}",
		...
		// inside the loop.
		"				vec3 rp = start_loc;",
		"				vec3 path_L = vec3(0.0,0.0,0.0);",
		"				int iSteps = int(length(step));",
		"				vec3 dd = step / float(iSteps);",
		"				step = normalize(step);",
		"				float tau = 0.0;",
		"				rp += 0.0*dd;",
		"				float rho0 = sampleDensity(rp), rho1;",
		"				float sigma_a = 0.1;",
		"				float sigma_s = 0.1;",
		"				float sigma_e = 1.0;",
		...
		// "						float val = sample1(loc);",
		"						rp += dd;",
		"						rho1 = sampleDensity(rp);",
		"						float rho = length(dd) * 0.5 * (rho0 + rho1);",
		"						tau += rho;",
		"						float transmittance = exp(-(sigma_a + sigma_s)*tau);",
		"						vec3 emission = vec3(0.0,0.0,0.0);",
		"						emission += apply_dvr_colormap(sampleData(u_gasData, rp),u_gasClip,u_gasClim,u_cmGasData,rho,0.0,rp);",
		"						emission += apply_dvr_colormap(sampleData(u_dmData, rp),u_dmClip,u_dmClim,u_cmDMData,rho,0.0,rp);",
		...
		// "						if((loc.x>u_xyzMin[0] && loc.x<u_xyzMax[0]) && (loc.y>u_xyzMin[1] && loc.y<u_xyzMax[1]) && (loc.z>u_xyzMin[2] && loc.z<u_xyzMax[2])){",
		// "							if(u_gasVisibility == true && u_dmVisibility == true){",
		// "								c_i_gas = apply_dvr_colormap(gasVal,u_gasClip,u_gasClim,u_cmGasData,density,0.0,loc);",
		// "								c_i_dm = apply_dvr_colormap(dmVal,u_dmClip,u_dmClim,u_cmDMData,density,0.0,loc);",
		// // "								c_i_gas.rgb *= c_i_gas.a;",
		// // "								c_i_dm.rgb *= c_i_dm.a;",
		...
		// "								}",
		// // "								else if(c_i_dm.a != 0.0 && c_i_gas.a == 0.0){",
		// // "									c_i = c_i_gas;",
		// // "								}",
		// // "								else if(c_i_dm.a == 0.0 && c_i_gas.a != 0.0){",
		// // "									c_i = c_i_dm;",
		...
		// // "								c_i.a = (c_i_gas.a+c_i_dm.a);",
		// // "								float c_i_r = c_i_gas.r*c.a + c_i_dm.r*(1.0-c.a);",
		// // "								float c_i_g = c_i_gas.g*c.a + c_i_dm.g*(1.0-c.a);",
		// // "								float c_i_b = c_i_gas.b*c.a + c_i_dm.b*(1.0-c.a);",
		// // "								float c_i_a = c_i_gas.a + c_i_dm.a*(1.0-c.a);",
		// // "								c_i = vec4(c_i_r,c_i_g,c_i_b,c_i_a);",
		// "							}",
		// "							else if(u_gasVisibility != true && u_dmVisibility == true){",
		// "								c_i_dm = apply_dvr_colormap(dmVal,u_dmClip,u_dmClim,u_cmDMData,density,0.0,loc);",
		// "								c_i = c_i_dm;",
		...
		// // "							c_i_a = 1.0 - pow((1.0 - c_i_a),(1.0/u_stepSize));",
		// "							c = vec4(c_i_r,c_i_g,c_i_b,c_i_a);",
		// "						}",
		// "						else{",
		// "							c_i = vec4(0.0156,0.0234,0.0898,0.1);",
		// "						}",
		...
		// Resolve final color
		"				vec3 L = vec3(1.0,1.0,1.0) - exp(-1.0*path_L);",
