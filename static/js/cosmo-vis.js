...
var camera, thumbCamera, scene, renderer, material, skewerScene //THREE.js environment variables
...
var gasMaterial, dmMaterial, starMaterial, starSaoMaterial, bhMaterial, skewerMaterial, starThumbMaterial
var skewerMaterial1 = []
var skewerTexture = []
var climGasLimits = []
var climDMLimits = []
...
var starScene, starSaoScene, skewerScene, starThumbScene
...
// list of redshifts in 12 Mpc box:
const zs = [
    0.0,
    0.101,
    0.183,
...
const lookBackTime = //in Gigayears
    [0.00,
        1.34,
        2.29,
        3.23,
...
const snapshotIDs = [
    'RefL0012N0188',
    'RefL0012N0188_snap027',
    'RefL0012N0188_snap026',
    'RefL0012N0188_snap025',
    'RefL0012N0188_snap024',
    'RefL0012N0188_snap023',
    'RefL0012N0188_snap022',
    'RefL0012N0188_snap021',
    'RefL0012N0188_snap020',
    'RefL0012N0188_snap019',
    'RefL0012N0188_snap018',
    'RefL0012N0188_snap017',
    'RefL0012N0188_snap016',
    'RefL0012N0188_snap015',
    'RefL0012N0188_snap014',
    'RefL0012N0188_snap013',
    'RefL0012N0188_snap012',
    'RefL0012N0188_snap011',
    'RefL0012N0188_snap010',
    'RefL0012N0188_snap009',
    'RefL0012N0188_snap008',
    'RefL0012N0188_snap007',
    'RefL0012N0188_snap006',
    'RefL0012N0188_snap005',
    'RefL0012N0188_snap004',
    'RefL0012N0188_snap003',
    'RefL0012N0188_snap002',
    'RefL0012N0188_snap001',
    'RefL0012N0188_snap000']

var dataArrays3D = [] 
var dataArrays3DZoom = []
var dataTextures3D = []
var dataTextures3DZoom = []
var volumesShader = []
var volumesShaderZoom = []
var volumesUniforms = []
var volumesUniformsZoom = []
var volsMaterial = []
var volsMaterialZoom = []
var volsGeometry = []
var volsGeometryZoom = []
var temporalEdges = []
var temporalStarData = []
var temporalStarPositions = []
var temporalStarGeometries = []
var loopPingPong = false
var loopDirection = 0
var play = true
var repeat = true
var started = false
...
var singleSnapshot = true
var temporalAnimation = false
/**
 * * GLOBAL FUNCTIONS 
 */

function singleSnapshotMode() {
    stopPlayback()
    hidePanel('redshift-select')
    singleSnapshot = true
    ...
    updateSize(zoom_bool = false)
    document.getElementById("singlesnap-radio").checked = true
}

function galaxyZoomMode(snapshot) {
    stopPlayback()
    hidePanel('redshift-select')
    singleSnapshot = true
    galaxyZoom = true
...
function temporalAnimationMode() {
    stopPlayback()
    showPanel('redshift-select')
    singleSnapshot = false
    temporalAnimation = true
    loadSnapSequence(snapshotIDs)
    setupThumbRenderTargets()
    setupThumbnailScenes()
    // startPlayback()
}

function zoomAnimationMode(snapshots) {
    stopPlayback()
    showPanel('redshift-select')
    singleSnapshot = false
    temporalAnimation = true
    galaxyZoom = true
    loadZoomSnapSequence(snapshots)
    // startPlayback()
}

/**
 * Load a sequence of snapshots for a simulation
 * @param {any} snapshotIDs is the list of snapshots
 * @returns {any}
 */
async function loadSnapSequence(snapshotIDs) {
    stopPlayback()
    gridsize = 64
    startLoadingAnimation()
    clearLayer(0)
    // init data textures for each snap
    init3dDataTextures(gridsize, snapshotIDs.length)
    // load data into textures
    for (let i = 0; i < snapshotIDs.length; i++) {
        var dens = await loadTemporalDensities(gridsize, 'PartType0', 'H_number_density', snapshotIDs, i)
        var gas = await loadTemporalGas(gridsize, gasAttr, true, snapshotIDs, i)
        var stars = await loadTemporalStars(snapshotIDs[i], i)
        // var update = await update3dDataTextures(i)
        // plotFrame(i)
    }
    // densityMin = dens[0]
    // densityMax = dens[1]
...
async function loadZoomSnapSequence(snapshots) {
    ...
    // clearLayer(0)
    // init data textures for each snap
    // init3dDataTextures(gridsize, snapshots.length)
    galSequenceCenters = []
    removeArrows()

    for(i=0;i<28;i++){
        dataArrays3DZoom[i] = new Float32Array(gridsize * gridsize * gridsize * 3)
        dataTextures3DZoom[i] = new THREE.DataTexture3D(dataArrays3DZoom[i], gridsize, gridsize, gridsize)
        dataTextures3DZoom[i].internalformat = 'RGB32F' //'RGB8UI' //
        dataTextures3DZoom[i].format = THREE.RGBFormat //RGBFormat//RGBIntegerFormat
        dataTextures3DZoom[i].type = THREE.FloatType ////UnsignedByteType 
        dataTextures3DZoom[i].minFilter = THREE.LinearFilter //LinearMipmapLinearFilter
        dataTextures3DZoom[i].magFilter = THREE.LinearFilter
        dataTextures3DZoom[i].unpackAlignment = 4
        dataTextures3DZoom[i].encoding = THREE.sRGBEncoding
        dataTextures3DZoom[i].anisotropy = 16
        dataTextures3DZoom[i].needsUpdate = true

        volumesUniformsZoom[i] = THREE.UniformsUtils.clone(volumeShader.uniforms);
        volumesUniformsZoom[i]["u_dataTexture3D"].value = dataTextures3DZoom[i];
        volumesUniformsZoom[i]["u_gasClip"].value = [true, true]
        volumesUniformsZoom[i]["u_dmClip"].value = [true, true]
        volumesUniformsZoom[i]["u_dmVisibility"].value = false

        volsMaterialZoom[i] = new THREE.ShaderMaterial({
            uniforms: volumesUniformsZoom[i],
            vertexShader: volumeShader.vertexShader,
            fragmentShader: volumeShader.fragmentShader,
            clipping: false,
            side: THREE.FrontSide, // The volume shader uses the backface as its "reference point"
            transparent: true,
        ...
        volsMaterialZoom[i].extensions.derivatives = true
        volsMaterialZoom[i].glslVersion = THREE.GLSL3

        updateUniforms()

        // if(thumbZoomScenes[i].thumbZoomScene.getObjectByName('Arrow')){
        //     while(thumbZoomScenes[i].thumbZoomScene.getObjectByName('Arrow')){
        //         thumbZoomScenes[i].thumbZoomScene.remove(thumbZoomScenes[i].thumbZoomScene.getObjectByName('Arrow'))
        //     }
        // }
    }
    // load data into textures
    for (let i = 0; i < snapshots.length; i++) {
        let snapID = snapshotIDs[snapshotIDs.length - snapshots[i][2] - 1]
        let haloID = snapshots[i][0]
        var dens = await loadTemporalZoomDensities(gridsize, 'PartType0', 'H_number_density', snapID, i, haloID)
        var gas = await loadTemporalZoomGas(gridsize, gasAttr, true, snapID, i, haloID)
        var stars = await loadTemporalZoomStars(haloID, snapID, i)
        // var update = await update3dDataTextures(i)
        // plotFrame(i)
    }
    // densityMin = dens[0]
...
/**
 * updatePlaybackRange sets the start and end points of the temporal visualization loop
 * @param {any} range is a tuple containing min and max values [min, max]
 * @returns {any}
 */
function updatePlaybackRange(range) {
    playbackRange = range
    playbackIndex = range[0]
}

/**
 * Description
 * @param {any} value in frames per second
 * @returns {any}
 */
...
function toggleLoopPingPong(checked) {
    if (checked === true) {
        loopPingPong = true
    }
    else {
 ...
 * @returns {any}
 */
function loopSequence() {         //  create a loop function
    // console.log(playbackIndex)
    started = true
    let l = playbackRange[1] - playbackRange[0]
    d3.select('p#current-z').text('Current snapshot: z = ' + zs[playbackIndex]);
    if (play === true) {
        setTimeout(function () {
            if (loopDirection == 0) {
                if (playbackIndex > playbackRange[0]) {
                    plotFrame(playbackIndex)
                    playbackIndex--
                    loopSequence();
                }
                else {
                    if (loopPingPong === true) {
                        loopDirection = 1;
                    }
                    else {
                        playbackIndex = playbackRange[1];
                    }
                    if (repeat === true) {
                        loopSequence()
                    }
                ...
                if (playbackIndex < playbackRange[1]) {
                    plotFrame(playbackIndex)
                    playbackIndex++
                    loopSequence();
                }
...
function resetCamera(){
    ...
    updateXYZDomain('x',0,1)
    updateXYZDomain('y',0,1)
    updateXYZDomain('z',0,1)
...
function resetGalaxyQuery(){

}

async function resetZoomToBigBox(){
    singleSnapshotMode()
}

/**
 * createRedshiftSelectionPanel updates the DOM to include UI elements for temporal visualization control
 * @returns {any}
 */
function createRedshiftSelectionPanel() {
    var sliderRange = d3.sliderBottom()
        ...
        .on('onchange', val => {
            let range = []
            let diffArr = zs.map(x => Math.abs(val[0] - x));
            let minNumber = Math.min(...diffArr);
            range[0] = diffArr.findIndex(x => x === minNumber)
            val[0] = zs[range[0]];
            diffArr = zs.map(x => Math.abs(val[1] - x));
            let maxNumber = Math.min(...diffArr);
            range[1] = diffArr.findIndex(x => x === maxNumber)
            val[1] = zs[range[1]];
            d3.select('p#value-range').text('Selected range: z = ' + val.map(d3.format('.3f')).join('-'));
            updatePlaybackRange(range)
        });
...
/**
 * Intialize arrays for 3D data textures for the sequence
 * @param {any} size is the gridsize
 * @param {any} amount is the number of snapshots in the sequence
 * @returns {any}
 */
function init3dDataTextures(size, amount) {
    clearLayer(0)
    ...
    return new Promise(resolve => {
        for (let i = 0; i < amount; i++) {
            dataArrays3D[i] = new Float32Array(size * size * size * 3) //Float32Array(size * size * size * 3)
            dataTextures3D[i] = new THREE.DataTexture3D(dataArrays3D[i], size, size, size)
            dataTextures3D[i].internalformat = 'RGB32F' //'RGB8UI' //
            dataTextures3D[i].format = THREE.RGBFormat //RGBFormat//RGBIntegerFormat
            dataTextures3D[i].type = THREE.FloatType ////UnsignedByteType 
            dataTextures3D[i].minFilter = THREE.LinearFilter //LinearMipmapLinearFilter
            dataTextures3D[i].magFilter = THREE.LinearFilter
            dataTextures3D[i].unpackAlignment = 4
            dataTextures3D[i].encoding = THREE.sRGBEncoding
            dataTextures3D[i].anisotropy = 16
            dataTextures3D[i].needsUpdate = true

            volumeShader = THREE.VolumeRenderShader;
            volumesUniforms[i] = THREE.UniformsUtils.clone(volumeShader.uniforms);
            volumesUniforms[i]["u_dataTexture3D"].value = dataTextures3D[i];
            volumesUniforms[i]["u_gasClip"].value = [true, true]
            volumesUniforms[i]["u_dmClip"].value = [true, true]
            volumesUniforms[i]["u_dmVisibility"].value = false


            volsMaterial[i] = new THREE.ShaderMaterial({
                uniforms: volumesUniforms[i],
                vertexShader: volumeShader.vertexShader,
                fragmentShader: volumeShader.fragmentShader,
                clipping: false,
                side: THREE.FrontSide, // The volume shader uses the backface as its "reference point"
            ...
            volsMaterial[i].extensions.derivatives = true
            volsMaterial[i].glslVersion = THREE.GLSL3

            dataArrays3DZoom[i] = new Float32Array(size * size * size * 3)
            dataTextures3DZoom[i] = new THREE.DataTexture3D(dataArrays3DZoom[i], size, size, size)
            dataTextures3DZoom[i].internalformat = 'RGB32F' //'RGB8UI' //
            dataTextures3DZoom[i].format = THREE.RGBFormat //RGBFormat//RGBIntegerFormat
            dataTextures3DZoom[i].type = THREE.FloatType ////UnsignedByteType 
            dataTextures3DZoom[i].minFilter = THREE.LinearFilter //LinearMipmapLinearFilter
...
/**
 * This function updates data textures and materials for each snapshot in the sequence after they are loaded
 * @param {any} index
 * @returns {any}
 */
function update3dDataTextures(index) {
    return new Promise(resolve => {
        // startLoadingAnimation()
        //console.log("update 3d texture uniforms")
        dataTextures3D[index] = new THREE.DataTexture3D(dataArrays3D[index], gridsize, gridsize, gridsize)
        dataTextures3D[index].internalformat = 'RGB32F' // 'RGB8UI'
        dataTextures3D[index].format = THREE.RGBFormat
        dataTextures3D[index].type = THREE.FloatType // UnsignedByteType // 
        dataTextures3D[index].minFilter = dataTextures3D[index].magFilter = THREE.LinearFilter //THREE.NearestFilter//
        dataTextures3D[index].unpackAlignment = 4
        dataTextures3D[index].needsUpdate = true
        volsMaterial[index].uniforms["u_dataTexture3D"].value = dataTextures3D[index];
        volsMaterial[index].uniforms["u_size"].value.set(gridsize, gridsize, gridsize);
        volsMaterial[index].uniforms["u_gasClim"].value.set(climGasLimits[0], climGasLimits[1]);
        volsMaterial[index].uniforms["u_dmClim"].value.set(climDMLimits[0], climDMLimits[1]);
        volsMaterial[index].uniforms["u_renderstyle"].value = 'mip' == 'mip' ? 0 : 1; // 0: MIP, 1: ISO
        volsMaterial[index].uniforms["u_renderthreshold"].value = 1.0; // For ISO renderstyle
        volsMaterial[index].uniforms["u_cmGasData"].value = cmtexture['PartType0'];
        volsMaterial[index].uniforms["u_cmDMData"].value = cmtexture['PartType1'];
        volsMaterial[index].extensions.drawBuffers = true
        volsMaterial[index].extensions.fragDepth = true
        volsMaterial[index].extensions.shaderTextureLOD = true
        volsMaterial[index].needsUpdate = true
        volsMaterial[index].uniforms["u_starDiffuse"].value = starTarget.texture
        volsMaterial[index].uniforms["u_starDepth"].value = starTarget.depthTexture
        volsMaterial[index].uniforms["u_glyphDiffuse"].value = starThumbTargets[index].texture
        volsMaterial[index].uniforms["u_glyphDepth"].value = starThumbTargets[index].depthTexture
        let size = new THREE.Vector2(window.innerWidth / 28, window.innerWidth / 28);
        volsMaterial[index].uniforms["u_glyphWidth"].value = new THREE.Vector2(size.x, size.y)
        volsMaterial[index].uniforms["u_glyphOffset"].value = new THREE.Vector2((27-index) * size.x, 4)
        volsMaterial[index].uniforms["u_glyphVisibility"].value = true


        volsGeometry[index] = new THREE.BoxBufferGeometry(gridsize, gridsize, gridsize);
        volsGeometry[index].translate(gridsize / 2, gridsize / 2, gridsize / 2);

        dataTextures3DZoom[index] = new THREE.DataTexture3D(dataArrays3DZoom[index], gridsize, gridsize, gridsize)
        dataTextures3DZoom[index].internalformat = 'RGB32F' // 'RGB8UI'
        dataTextures3DZoom[index].format = THREE.RGBFormat
        dataTextures3DZoom[index].type = THREE.FloatType // UnsignedByteType // 
        dataTextures3DZoom[index].minFilter = dataTextures3DZoom[index].magFilter = THREE.LinearFilter //THREE.NearestFilter//
        dataTextures3DZoom[index].unpackAlignment = 4
        dataTextures3DZoom[index].needsUpdate = true
        volsMaterialZoom[index].uniforms["u_dataTexture3D"].value = dataTextures3DZoom[index];
        volsMaterialZoom[index].uniforms["u_size"].value.set(gridsize, gridsize, gridsize);
        volsMaterialZoom[index].uniforms["u_gasClim"].value.set(climGasLimits[0], climGasLimits[1]);
        volsMaterialZoom[index].uniforms["u_dmClim"].value.set(climDMLimits[0], climDMLimits[1]);
        volsMaterialZoom[index].uniforms["u_renderstyle"].value = 'mip' == 'mip' ? 0 : 1; // 0: MIP, 1: ISO
        volsMaterialZoom[index].uniforms["u_renderthreshold"].value = 1.0; // For ISO renderstyle
        volsMaterialZoom[index].uniforms["u_cmGasData"].value = cmtexture['PartType0'];
        volsMaterialZoom[index].uniforms["u_cmDMData"].value = cmtexture['PartType1'];
        volsMaterialZoom[index].extensions.drawBuffers = true
        volsMaterialZoom[index].extensions.fragDepth = true
        volsMaterialZoom[index].extensions.shaderTextureLOD = true
        volsMaterialZoom[index].needsUpdate = true
        volsMaterialZoom[index].uniforms["u_starDiffuse"].value = starTarget.texture
        volsMaterialZoom[index].uniforms["u_starDepth"].value = starTarget.depthTexture

        volsGeometryZoom[index] = new THREE.BoxBufferGeometry(gridsize, gridsize, gridsize);
        volsGeometryZoom[index].translate(gridsize / 2, gridsize / 2, gridsize / 2);
        updateUniforms()
        resolve()
    })
}
...
var currentFrame = 0
/**
 * Updates scene based on snapshot index
 * @param {any} index
 * @returns {any}
 */
function plotFrame(index) {
    ...
    boxOfStarPoints = new THREE.Points(temporalStarGeometries[index], starMaterial);
    ...
    if(!galaxyZoom){
        var mesh = new THREE.Mesh(volsGeometry[currentFrame], volsMaterial[currentFrame]);

    }
    if(galaxyZoom){
        var mesh = new THREE.Mesh(volsGeometryZoom[currentFrame], volsMaterialZoom[currentFrame]);
    }
    scene.add(mesh);
    mesh.layers.set(0)
    mesh.renderOrder = 2
    ...
    updateUniforms()
    update3dDataTextures(currentFrame)
    // var dataArray3DOld = dataArrays3D[oldFrame].slice()

    // const tween = new TWEEN.Tween(dataArrays3D[oldFrame])
    // .to(dataArrays3D[currentFrame], 10)
    // .onUpdate(() => {            
    //     updateUniforms()
    //     update3dDataTextures(oldFrame)  }).start()

    
    // dataArrays3D[oldFrame] = dataArray3DOld.slice()
    //interpolate
    // dataArrays3D[index]
...
/**
 * Load densities for temporal visualization
 * @param {any} size is the gridsize
 * @param {any} type is the particle type
 * @param {any} attr is the attribute
 * @param {any} snapshotIDs is the list of snapshots in the sequence
 * @returns {any}
 */
function loadTemporalDensities(size, type, attr, snapshotIDs, index) {
    // startLoadingAnimation()
    return new Promise(resolve => {
        let simID = snapshotIDs[index]
        d3.json('static/data/' + simID + '/' + type + '/' + size + '_' + type + '_' + attr + '.json').then(function (d) {
            ...
            for (x = 0; x < size; x++) {
                for (y = 0; y < size; y++) {
                    for (z = 0; z < size; z++) {
                        ...
                        dataArrays3D[index][3 * (x + y * size + z * size * size) + 2] = d[x][y][z]
                        ...
                        if (dataArrays3D[index][3 * (x + y * size + z * size * size) + 2] < min) {
                            min = dataArrays3D[index][3 * (x + y * size + z * size * size) + 2]
                        }
                        if (dataArrays3D[index][3 * (x + y * size + z * size * size) + 2] > max) {
                            max = dataArrays3D[index][3 * (x + y * size + z * size * size) + 2]
                        }
                    }
                }
            }
            ...
            if (simID != "TNG100_z2.3") {
                ...
                volsMaterial[index].uniforms["u_densityUnpackDomain"].value.set(densityUnpackDomain[0], densityUnpackDomain[1])
            }
 ...
 * @returns {any}
 */
function loadTemporalZoomDensities(size, type, attr, snapID, index, haloID) {
    // startLoadingAnimation()
    return new Promise(resolve => {
        let simID = snapID
        // d3.json('static/data/' + simID + '/' + type + '/' + size + '_' + type + '_' + attr + '.json').then(function (d) {
        d3.json('static/data/' + simID + '/Halos/halo_' + haloID + '/' + type + '/' + size + '_PartType0_' + attr + '.json').then(function (d) {
            ...
            for (x = 0; x < size; x++) {
                for (y = 0; y < size; y++) {
                    for (z = 0; z < size; z++) {
                        // if(size==64) dataArray3D[ 3 * ( x + y * size + z * size * size ) + 2 ] = Math.log10(d[x][y][z])
                        dataArrays3DZoom[index][3 * (x + y * size + z * size * size) + 2] = d[x][y][z]

                        // if(size==384 || size==512) dataArray3D[ 4 * ( x + y * size + z * size * size ) + 2 ] = d[x][y][z]
                        // else dataArray3D[ 4 * ( x + y * size + z * size * size ) + 2 ] = Math.log10(d[x][y][z])

                        if (dataArrays3DZoom[index][3 * (x + y * size + z * size * size) + 2] < min) {
                            min = dataArrays3DZoom[index][3 * (x + y * size + z * size * size) + 2]
                        }
                        if (dataArrays3DZoom[index][3 * (x + y * size + z * size * size) + 2] > max) {
                            max = dataArrays3DZoom[index][3 * (x + y * size + z * size * size) + 2]
                        }
                    }
                }
            }
            ...
            if (simID != "TNG100_z2.3") {
                densityUnpackDomain = [-8.5, -3]
                setDensityMinMaxInputValues('density', -8.5, -3)
                volsMaterialZoom[index].uniforms["u_densityUnpackDomain"].value.set(densityUnpackDomain[0], densityUnpackDomain[1])
            }
...
/**
 * Load gas data for temporal visualization
 * @param {any} size
 * @param {any} attr
 * @param {any} resolution_bool
 * @param {any} snapshotIDs
 * @param {any} index
 * @returns {any}
 */
function loadTemporalGas(size, attr, resolution_bool, snapshotIDs, index) {
    // startLoadingAnimation()
    gasAttr = attr
    ...
    return new Promise(resolve => {
        ...
        d3.json('static/data/' + simID + '/PartType0/' + size + '_PartType0_' + attr + '.json').then(function (d) {
            ...
            volsMaterial[index].uniforms["u_gasAttribute"] = attr
                ...
                for (y = 0; y < size; y++) {
                    for (z = 0; z < size; z++) {
                        if (simID == "TNG100" && attr == "GFM_Metallicity") {
                            dataArrays3D[index][3 * (x + y * size + z * size * size)] = d[x][y][z]
                        }
                        // else if(attr=="Metallicity"){
                        //     dataArray3D[ 3 * ( x + y * size + z * size * size ) ] =  Math.log10(d[x][y][z]*77.22015)
                        // }
                        else if (log) {
                            dataArrays3D[index][3 * (x + y * size + z * size * size)] = Math.log10(d[x][y][z])
                        } else {
                            dataArrays3D[index][3 * (x + y * size + z * size * size)] = d[x][y][z]
                        }

                        if (dataArrays3D[index][3 * (x + y * size + z * size * size)] < min) {
                            min = dataArrays3D[index][3 * (x + y * size + z * size * size)]
                        }
                        if (dataArrays3D[index][3 * (x + y * size + z * size * size)] > max) {
                            max = dataArrays3D[index][3 * (x + y * size + z * size * size)]
                        }
            ...
            volsMaterial[index].uniforms["u_gasUnpackDomain"].value.set(gasUnpackDomain[0], gasUnpackDomain[1])
 ...
 * @returns {any}
 */
function loadTemporalZoomGas(size, attr, resolution_bool, snapID, index, haloID) {
    gasAttr = attr
    //console.log('loading gas')
    return new Promise(resolve => {
        let simID = snapID
        ...
        d3.json('static/data/' + simID + '/Halos/halo_' + haloID + '/PartType0/' + size + '_PartType0_' + attr + '.json').then(function (d) {
            volsMaterialZoom[index].uniforms["u_gasAttribute"] = attr
            ...
            for (x = 0; x < size; x++) {
                for (y = 0; y < size; y++) {
                    for (z = 0; z < size; z++) {
                        if (simID == "TNG100" && attr == "GFM_Metallicity") {
                            dataArrays3DZoom[index][3 * (x + y * size + z * size * size)] = d[x][y][z]
                        }
                        // else if(attr=="Metallicity"){
                        //     dataArray3D[ 3 * ( x + y * size + z * size * size ) ] =  Math.log10(d[x][y][z]*77.22015)
                        // }
                        else if (log) {
                            dataArrays3DZoom[index][3 * (x + y * size + z * size * size)] = Math.log10(d[x][y][z])
                        } else {
                            dataArrays3DZoom[index][3 * (x + y * size + z * size * size)] = d[x][y][z]
                        }

                        if (dataArrays3DZoom[index][3 * (x + y * size + z * size * size)] < min) {
                            min = dataArrays3DZoom[index][3 * (x + y * size + z * size * size)]
                        }
                        if (dataArrays3DZoom[index][3 * (x + y * size + z * size * size)] > max) {
                            max = dataArrays3DZoom[index][3 * (x + y * size + z * size * size)]
                        }
            ...
            volsMaterialZoom[index].uniforms["u_gasUnpackDomain"].value.set(gasUnpackDomain[0], gasUnpackDomain[1])
            // if(elements.includes(attr)){
...
/**
 * Load star data for temporal visualization
 * @param {any} snapshotID
 * @param {any} index
 * @returns {any}
 */
function loadTemporalStars(snapshotID, index) {
    // startLoadingAnimation()
    //console.log('loading stars')
    console.log(snapshotID, index)
    return new Promise(resolve => {
        // while (starScene.children.length > 0) {
        //     starScene.remove(starScene.children[0]);
        // }

        d3.json('static/data/' + snapshotID + '/simMetadata.json').then(function (d) {
            temporalEdges[index] = []
            temporalEdges[index].left_edge = d.left_edge
            temporalEdges[index].right_edge = d.right_edge

            if (snapshotID !== 'RefL0012N0188_snap000') {

                d3.json('static/data/' + snapshotID + '/PartType4/star_particles.json').then(function (d) {
                    ...
                    if (Object.keys(d).length > 0) {
                        for (let i = 0; i < n; i++) {
                            mx = gridsize * ((d[i][0] - temporalEdges[index].left_edge[0]) / (temporalEdges[index].right_edge[0] - temporalEdges[index].left_edge[0]))
                            my = gridsize * ((d[i][1] - temporalEdges[index].left_edge[1]) / (temporalEdges[index].right_edge[1] - temporalEdges[index].left_edge[1]))
                            mz = gridsize * ((d[i][2] - temporalEdges[index].left_edge[2]) / (temporalEdges[index].right_edge[2] - temporalEdges[index].left_edge[2]))
                        ...
                        temporalStarPositions[index] = starPositions
                        temporalStarData[index] = starData
                        // console.log('star positions:')
                        // console.log(starPositions)
                        starGeometry.setAttribute('position', new THREE.Float32BufferAttribute(temporalStarPositions[index], 3)) //.onUpload( disposeArray )
                        starGeometry.computeBoundingBox()

                        temporalStarGeometries[index] = starGeometry
...
var temporalZoomEdges = []
var galSequenceCenters = []
var temporalSpinArrow = []
/**
 * Load star data for temporal visualization
 * @param {any} snapshotID
 * @param {any} index
 * @returns {any}
 */
function loadTemporalZoomStars(haloID, snapID, index) {
    // startLoadingAnimation()
    //console.log('loading stars')
    console.log(snapID, index)
    return new Promise(resolve => {
        // while (starScene.children.length > 0) {
        //     starScene.remove(starScene.children[0]);
        // }
        ...
        d3.json('static/data/' + snapID + '/Halos/halo_' + haloID + '/simMetadata.json').then(function (d) {
            temporalZoomEdges[index] = []
            temporalZoomEdges[index].left_edge = d.left_edge
            temporalZoomEdges[index].right_edge = d.right_edge

            galCenter = d.star_center
            galSpin = d.star_spin

            galSequenceCenters[index] = galCenter
            console.log(galSequenceCenters[index])


            d3.json('static/data/' + snapID + '/Halos/halo_' + haloID + '/PartType4/star_particles.json').then(function (d) {
                // //console.log( Object.keys(d).length )
                var starData = []
                n = Object.keys(d).length
                var starGeometry = new THREE.BufferGeometry();
                var starPositions = new Float32Array(n * 3)
                if (Object.keys(d).length > 0) {
                    console.log(temporalZoomEdges[index])
                    for (let i = 0; i < n; i++) {
                        let mx = gridsize * ((d[i][0] - temporalZoomEdges[index].left_edge[0]) / (temporalZoomEdges[index].right_edge[0] - temporalZoomEdges[index].left_edge[0]))
                        let my = gridsize * ((d[i][1] - temporalZoomEdges[index].left_edge[1]) / (temporalZoomEdges[index].right_edge[1] - temporalZoomEdges[index].left_edge[1]))
                        let mz = gridsize * ((d[i][2] - temporalZoomEdges[index].left_edge[2]) / (temporalZoomEdges[index].right_edge[2] - temporalZoomEdges[index].left_edge[2]))
                        let vertex = new THREE.Vector3(mx, my, mz)
                        vertex.toArray(starPositions, i * 3)
                        starData[i] = [d[i][0], //x
                        d[i][1], //y
                ...
                galX = gridsize * ((galCenter[0] - temporalZoomEdges[index].left_edge[0]) / (temporalZoomEdges[index].right_edge[0] - temporalZoomEdges[index].left_edge[0]))
                galY = gridsize * ((galCenter[1] - temporalZoomEdges[index].left_edge[1]) / (temporalZoomEdges[index].right_edge[1] - temporalZoomEdges[index].left_edge[1]))
                galZ = gridsize * ((galCenter[2] - temporalZoomEdges[index].left_edge[2]) / (temporalZoomEdges[index].right_edge[2] - temporalZoomEdges[index].left_edge[2]))
                ...
                var spinLength = length// ((128 * length) / gridsize)
                ...
                temporalSpinArrow[index] = new THREE.ArrowHelper(spinDirection.normalize(), from, spinLengthX, 0xffffff)
                temporalSpinArrow[index].name = "Arrow"
                var x = document.getElementById("star-eye-open");
                x.style.display = "inline-block";
                var y = document.getElementById("star-eye-closed");
                y.style.display = "none";
...
function updateUniforms() {
    ...
    if (temporalAnimation === true) {

        for (let index = 0; index < snapshotIDs.length; index++) {

            if(volsMaterialZoom[index]){
                volsMaterialZoom[index].uniforms["u_screenHeight"].value = window.innerHeight
                volsMaterialZoom[index].uniforms["u_screenWidth"].value = window.innerWidth
                volsMaterialZoom[index].uniforms["u_exposure"].value = document.getElementById("exposure").value
                volsMaterialZoom[index].uniforms["u_xyzMin"].value = new THREE.Vector3(domainXYZ[0], domainXYZ[2], domainXYZ[4])
                volsMaterialZoom[index].uniforms["u_xyzMax"].value = new THREE.Vector3(domainXYZ[1], domainXYZ[3], domainXYZ[5])
                volsMaterialZoom[index].uniforms["u_valModI"].value = (document.getElementById("val-mod-intensity")).value
                ...
                if (d_mod == 1.0) {
                    volsMaterialZoom[index].uniforms["u_density"].value = densityTexture;
                    // volMaterial.uniforms[ " u_grayscaleDepthMod" ]
                    // //console.log(densityMin,densityMax)
                    volsMaterialZoom[index].uniforms["u_climDensity"].value.set(densityMin, densityMax);
                } else {
                    volsMaterialZoom[index].uniforms["u_density"].value = blankTexture;
                    volsMaterialZoom[index].uniforms["u_climDensity"].value.set(1.0, 1.0);
                    // d = []
                }
                volsMaterialZoom[index].uniforms["u_densityDepthMod"].value = d_mod;
                volsMaterialZoom[index].uniforms["u_densityModI"].value = (document.getElementById("density-mod-intensity")).value
                volsMaterialZoom[index].uniforms["u_densityMod"].value = 1.0;
                if ((document.getElementById("val-mod-check")).checked) {
                    volsMaterialZoom[index].uniforms["u_valMod"].value = 1.0;
                } else {
                    volsMaterialZoom[index].uniforms["u_valMod"].value = 0.0;
                }
                if (document.getElementById("gas-min-check").checked && document.getElementById("gas-max-check").checked) {
                    ...
                    volsMaterialZoom[index].uniforms["u_gasClim"].value.set(climGasLimits[0], document.querySelector('#gas-maxval-input').value);
                } else if (!document.getElementById("gas-min-check").checked && document.getElementById("gas-max-check").checked) {
                    volsMaterialZoom[index].uniforms["u_gasClim"].value.set(document.querySelector('#gas-minval-input').value, climGasLimits[1]);
                } else if (!document.getElementById("gas-min-check").checked && !document.getElementById("gas-max-check").checked) {
                    volsMaterialZoom[index].uniforms["u_gasClim"].value.set(document.querySelector('#gas-minval-input').value, document.querySelector('#gas-maxval-input').value);
                } else {

                }
                volsMaterialZoom[index].uniforms["u_gasClip"].value = [document.getElementById("gas-min-clip-check").checked, document.getElementById("gas-max-clip-check").checked]
                ...
                gasColData = []
                volsMaterialZoom[index].uniforms["u_cmGasData"].value = cmtexture['PartType0'];
                volsMaterialZoom[index].uniforms["u_dmClip"].value = [document.getElementById("dm-min-clip-check").checked, document.getElementById("dm-max-clip-check").checked]
            ...
            if (volsMaterial[index]) {
                ...
                volsMaterial[index].uniforms["u_screenHeight"].value = window.innerHeight
                volsMaterial[index].uniforms["u_screenWidth"].value = window.innerWidth
                ...
                volsMaterial[index].uniforms["u_exposure"].value = document.getElementById("exposure").value

                //cutting sliders
                volsMaterial[index].uniforms["u_xyzMin"].value = new THREE.Vector3(domainXYZ[0], domainXYZ[2], domainXYZ[4])
                volsMaterial[index].uniforms["u_xyzMax"].value = new THREE.Vector3(domainXYZ[1], domainXYZ[3], domainXYZ[5])
                ...
                volsMaterial[index].uniforms["u_valModI"].value = (document.getElementById("val-mod-intensity")).value
                ...
                densityMax = document.getElementById('density-maxval-input').value
                if (d_mod == 1.0) {
                    volsMaterial[index].uniforms["u_density"].value = densityTexture;
                    // volMaterial.uniforms[ " u_grayscaleDepthMod" ]
                    // //console.log(densityMin,densityMax)
                    volsMaterial[index].uniforms["u_climDensity"].value.set(densityMin, densityMax);
                } else {
                    volsMaterial[index].uniforms["u_density"].value = blankTexture;
                    volsMaterial[index].uniforms["u_climDensity"].value.set(1.0, 1.0);
                    // d = []
                }
                volsMaterial[index].uniforms["u_densityDepthMod"].value = d_mod;
                volsMaterial[index].uniforms["u_densityModI"].value = (document.getElementById("density-mod-intensity")).value
                volsMaterial[index].uniforms["u_densityMod"].value = 1.0;
                ...
                if ((document.getElementById("val-mod-check")).checked) {
                    volsMaterial[index].uniforms["u_valMod"].value = 1.0;
                } else {
                    volsMaterial[index].uniforms["u_valMod"].value = 0.0;
                }
                ...
                if (document.getElementById("gas-min-check").checked && document.getElementById("gas-max-check").checked) {
                    ...
                    volsMaterial[index].uniforms["u_gasClim"].value.set(climGasLimits[0], document.querySelector('#gas-maxval-input').value);
                } else if (!document.getElementById("gas-min-check").checked && document.getElementById("gas-max-check").checked) {
                    volsMaterial[index].uniforms["u_gasClim"].value.set(document.querySelector('#gas-minval-input').value, climGasLimits[1]);
                } else if (!document.getElementById("gas-min-check").checked && !document.getElementById("gas-max-check").checked) {
                    volsMaterial[index].uniforms["u_gasClim"].value.set(document.querySelector('#gas-minval-input').value, document.querySelector('#gas-maxval-input').value);
                } else {

                }

                volsMaterial[index].uniforms["u_gasClip"].value = [document.getElementById("gas-min-clip-check").checked, document.getElementById("gas-max-clip-check").checked]
                ...
                if (document.getElementById("dm-min-check").checked && document.getElementById("dm-max-check").checked) {
                    ...
                    volsMaterial[index].uniforms["u_dmClim"].value.set(climDMLimits[0], document.querySelector('#dm-maxval-input').value);
                } else if (!document.getElementById("dm-min-check").checked && document.getElementById("dm-max-check").checked) {
                    volsMaterial[index].uniforms["u_dmClim"].value.set(document.querySelector('#dm-minval-input').value, climDMLimits[1]);
                } else if (!document.getElementById("dm-min-check").checked && !document.getElementById("dm-max-check").checked) {
                    volsMaterial[index].uniforms["u_dmClim"].value.set(document.querySelector('#dm-minval-input').value, document.querySelector('#dm-maxval-input').value);
                } else {

                }
                // //console.log(volMaterial.uniforms["u_dmClim"].value)
                volsMaterial[index].uniforms["u_dmClip"].value = [document.getElementById("dm-min-clip-check").checked, document.getElementById("dm-max-clip-check").checked]
...
function update3dDataTexture() {
    return new Promise(resolve => {
        ...
        volMaterial.uniforms["u_cmGasData"].value = cmtexture['PartType0'];
        volMaterial.uniforms["u_cmDMData"].value = cmtexture['PartType1'];
        let size = new THREE.Vector2(window.innerWidth, window.innerWidth);
        volMaterial.uniforms["u_glyphWidth"].value = new THREE.Vector2(size.x, size.y)
        volMaterial.uniforms["u_glyphOffset"].value = new THREE.Vector2(0,0)
        volMaterial.uniforms["u_glyphVisibility"].value = false
...
/**
 * Loads data asyncronously
 * @param {any} resolution_bool whether or not the grid size has changed
 * @returns {any}
 */
    ...
    return new Promise(resolve => {
        try {
            d3.json('static/data/' + simID + '/PartType1/' + size + '_PartType1_' + attr + '.json').then(function (d) {
                ...
                for (let i = 0; i < dmUnits.length; i++) {
                    if (simID == "RefL0100N1504") {
...
var haloSequences = {}
async function getGalaxySequences() {
    let galaxyInfo = []
    // load all of the galaxy metadata for the snapshot sequence
    galaxyInfo = await d3.json('static/data/' + simID + '_galaxySequence.json')
    // for each galaxy, traverse merger tree to map all descendantIDs (galaxyID) to get the corresponding haloID
    console.log(galaxyInfo)
    for (let i = 0; i < galaxyInfo.length; i++) {
        galaxyInfo[i].galID
        galaxyInfo[i].desHaloID
        galaxyInfo[i].progHaloID
        galaxyInfo[i].progMassType_Star
        galaxyInfo[i].progSnapNum

        // check if halo exists in dictionary yet, if not initialize it
        if (!haloSequences[galaxyInfo[i].desHaloID]) {
            haloSequences[galaxyInfo[i].desHaloID] = [];
        }
        // check if there is a duplicate
        let duplicate = false
        for (j = 0; j < haloSequences[galaxyInfo[i].desHaloID].length; j++) {
            // check if there is a duplicate entry
            if (JSON.stringify([galaxyInfo[i].progHaloID, galaxyInfo[i].progMassType_Star, galaxyInfo[i].progSnapNum]) == JSON.stringify(haloSequences[galaxyInfo[i].desHaloID][j])) {
                duplicate = true
            }
            // check if halo progenator from the same snapshot is in the sequence
            if (galaxyInfo[i].progSnapNum == haloSequences[galaxyInfo[i].desHaloID][j][2]) {
                // pick galaxy with the larger stellar mass
                duplicate = true
                if (galaxyInfo[i].progMassType_Star >= haloSequences[galaxyInfo[i].desHaloID][j][1]) {
                    haloSequences[galaxyInfo[i].desHaloID][j] = [galaxyInfo[i].progHaloID, galaxyInfo[i].progMassType_Star, galaxyInfo[i].progSnapNum]
                }
            }
        }
        if (!duplicate) {
            haloSequences[galaxyInfo[i].desHaloID].push([galaxyInfo[i].progHaloID, galaxyInfo[i].progMassType_Star, galaxyInfo[i].progSnapNum])
        }
        for (let h in haloSequences) {
            haloSequences[h] = haloSequences[h].sort((a, b) => b[2] - a[2])
        }
    }

    //return list of all of the halo ids and their sequence
    // {19000000000003: [... , ..., ...]}
}
...
async function starCaster() {
    ...
    let d = starGalaxyInfo

    if (d !== undefined) {
        var filteredData = d.slice() //slice of data
...
var thumbScenes = [];
var thumbZoomScenes = [];
// var bigBoxVolsGeometry, bigBoxVolsMaterial = [];


async function setupThumbnailScenes() {
    setupStarThumbScenes()
    // removeArrows()
    // if(!zoom_bool){
        thumbScenes = []
        thumbZoomScenes = [];
        for (let i = 0; i <= 28; i++) {
            ...
            let thumbScene = new THREE.Scene();
            thumbScene.background = new THREE.Color("rgb(4,6,23)")
            let mesh = new THREE.Mesh(volsGeometry[i], volsMaterial[i]);
            thumbScene.add(mesh);
            thumbScenes.push({ thumbScene, thumbCamera });
        }
    // }
    ...
    if (zoom_bool == true){
        ...
        for (let i = 0; i <= 28; i++) {
            // const camera = new THREE.PerspectiveCamera(75, 1, 0.1, 1000);
            let thumbZoomScene = new THREE.Scene();
            thumbZoomScene.background = new THREE.Color("rgb(4,6,23)")
            ...
            let mesh = new THREE.Mesh(volsGeometryZoom[i], volsMaterialZoom[i]);
            thumbZoomScene.add(mesh);
            // Add galaxy spin arrows
            if(temporalSpinArrow[i]){
                thumbZoomScene.add(temporalSpinArrow[i]);
            }
            thumbZoomScenes.push({ thumbZoomScene, thumbCamera });
            
            // remove old spheres
            // while(starThumbScenes[i].getObjectByName('Sphere')){
            //     starThumbScenes[i].remove(starThumbScenes[i].getObjectByName('Sphere'))

            // }

            // Draw sphere in big box thumbnail to highlight galaxy location
            if(galSequenceCenters[i]){
                let dl =  Math.sqrt((temporalZoomEdges[i].right_edge[0] - temporalZoomEdges[i].left_edge[0])^2
                                + (temporalZoomEdges[i].right_edge[1] - temporalZoomEdges[i].left_edge[1])^2
                                + (temporalZoomEdges[i].right_edge[2] - temporalZoomEdges[i].left_edge[2])^2)
                const geometry = new THREE.SphereGeometry(dl * 4, 32, 16);
                geometry.frustumCulled = false
                // const material = new THREE.MeshBasicMaterial({ color: 0x00ff00, transparent: false, opacity: 1.0, wireframe: true, side: THREE.FrontSide, depthWrite: false, depthTest: false});
                // material.frustumCulled = false
                // material.renderorder = 0
                // material.depthWrite = false
                // material.depthTest = false

                const sphere = new THREE.Mesh(geometry, starThumbMaterial);
                sphere.name = 'Sphere'
                galX = gridsize * ((galSequenceCenters[i][0] - temporalEdges[i].left_edge[0]) / (temporalEdges[i].right_edge[0] - temporalEdges[i].left_edge[0]))
                galY = gridsize * ((galSequenceCenters[i][1] - temporalEdges[i].left_edge[1]) / (temporalEdges[i].right_edge[1] - temporalEdges[i].left_edge[1]))
                galZ = gridsize * ((galSequenceCenters[i][2] - temporalEdges[i].left_edge[2]) / (temporalEdges[i].right_edge[2] - temporalEdges[i].left_edge[2]))
    
                sphere.position.x = galX
                sphere.position.y = galY
                sphere.position.z = galZ
                starThumbScenes[i].add(sphere);
                thumbScenes[i].thumbScene.add(sphere);
...
function removeThumbnailScenes(){

}

function removeArrows(){
    return new Promise(resolve => {
        for (let i = 0; i < 28; i++) {if(thumbZoomScenes.length > 0 && typeof thumbZoomScenes[i] !== 'undefined'){
            while(thumbZoomScenes[i].thumbZoomScene.getObjectByName('Arrow')){
                thumbZoomScenes[i].thumbZoomScene.remove(thumbZoomScenes[i].thumbZoomScene.getObjectByName('Arrow'))
            }    
        }}
...
function setupMinimapScene() {
    // when galaxy zoom in is loaded, show position in full volume
    // thumbScenes = []
    // for (let i = 0; i < 28; i++) {
    //     // const camera = new THREE.PerspectiveCamera(75, 1, 0.1, 1000);
    //     let thumbScene = new THREE.Scene();
...
starThumbScenes = []
function setupStarThumbScenes() {

    for(i=0;i<28;i++){
        starThumbScenes[i] = new THREE.Scene();

        starThumbScenes[i].background = new THREE.Color("rgb(0,0,0)")

    }
    // starCol = new THREE.Color(0.8, 0.8, 0.0)

    // //console.log(starCol)
    starThumbMaterial = new THREE.ShaderMaterial({

        uniforms: {
            Col: { value: new THREE.Color("rgb(0,255,0)") },
        ...
        vertexShader: document.getElementById('vertexshader-thumb').textContent,
        fragmentShader: document.getElementById('fragmentshader-thumb').textContent,
...
function setupStarScene() {
    ...
    // starCol = new THREE.Color(0.8, 0.8, 0.0)
    starCol = new THREE.Color(1.0, 1.0, 0.0)
...
starThumbTargets = []
function setupThumbRenderTargets(){
    for(let i = 0; i<28; i++){
        // if (starThumbTargets[i].starThumbTarget) starThumbTargets[i].starThumbTarget.dispose();
        ...
        starThumbTargets[i] = new THREE.WebGLRenderTarget(size.x,size.y);
        starThumbTargets[i].texture.format = THREE.RGBAFormat;
        starThumbTargets[i].texture.minFilter = THREE.LinearFilter;
        starThumbTargets[i].texture.magFilter = THREE.LinearFilter;
        starThumbTargets[i].texture.generateMipMaps = true
        starThumbTargets[i].stencilBuffer = (format === THREE.DepthStencilFormat) ? true : false;
        starThumbTargets[i].depthBuffer = true;
        starThumbTargets[i].depthTexture = new THREE.DepthTexture();
        starThumbTargets[i].depthTexture.format = format
        starThumbTargets[i].depthTexture.type = type;
        starThumbTargets[i].scissorTest = true;
        starThumbTargets[i].scissor
    }
}
...
//         for (let i = 0; i < Object.keys(galaxy_centers).length; i++) {
...
function goToPoint(x, y, z, delta = 0.1) {
    console.log(x, y, z, delta)
    //console.log('click click')
    // x*=0.6776999078
    // y*=0.6776999078
    // z*=0.6776999078

    width_Mpc_x = (edges.right_edge[0] - edges.left_edge[0])
    width_Mpc_y = (edges.right_edge[1] - edges.left_edge[1])
    width_Mpc_z = (edges.right_edge[2] - edges.left_edge[2])


    //     delta = 0.1
    domainXYZ[0] = (x / width_Mpc_x) - delta
    domainXYZ[1] = (x / width_Mpc_x) + delta
    domainXYZ[2] = (y / width_Mpc_y) - delta
    domainXYZ[3] = (y / width_Mpc_y) + delta
    domainXYZ[4] = (z / width_Mpc_z) - delta
    domainXYZ[5] = (z / width_Mpc_z) + delta

    console.log(domainXYZ)
    ...
    let margin = { top: 20, right: 15, bottom: 30, left: 20 };
    let width = 343 - margin.left - margin.right,
        height = 40

    xrange = [width * domainXYZ[0], width * domainXYZ[1]]
    console.log(xrange)
    var x = d3.scaleLinear()
        .domain([0.0, 1.0])
        .range([margin.left + width * domainXYZ[0], margin.left + width * domainXYZ[1]]);
...
function updateCameraNearAndFar() {
    ...
    if(temporalAnimation){
        for (let i = 0; i < snapshotIDs.length; i++) {
            if (volsMaterial[i]) {
                volsMaterial[i].uniforms["u_cameraNear"].value = camera.near
                volsMaterial[i].uniforms["u_cameraFar"].value = camera.far
                volsMaterial[i].needsUpdate = true
            }
...
function render() {
    ...
    // render loop if temporal mode is on
    if (temporalAnimation === true) {
        ...
        if (starSaoTarget) {
            ...
            if (volsMaterial) {
                ...
                volsMaterial[selectedSnapshotIndex].uniforms["u_starDiffuse"].value = starSaoTarget.texture
                // volMaterial.needsUpdate = true
            }
            renderer.setRenderTarget(null)
        }
        ...
        if (skewerTarget) {
            ...
            if (volsMaterial[selectedSnapshotIndex]) {
                volsMaterial[selectedSnapshotIndex].uniforms["u_skewerDiffuse"].value = skewerTarget.texture
                volsMaterial[selectedSnapshotIndex].uniforms["u_skewerDepth"].value = skewerTarget.depthTexture
            }
            renderer.setRenderTarget(null)
        }
    ...
    // render mode if single snapshot mode is on
    if (singleSnapshot === true) {
        renderer.setScissorTest(true)
        renderer.setScissor(0, 0, window.innerWidth, window.innerHeight);
        renderer.setViewport(0, 0, window.innerWidth, window.innerHeight);
    ...
    // big-box render loop for temporal thumbnails
    if(thumbs == true && temporalAnimation == true){
        let size = new THREE.Vector2(window.innerWidth / 28, window.innerWidth / 28);
        thumbScenes.forEach(({ thumbScene, thumbCamera }, index) => {
            if (temporalAnimation === true) {
                if(index === playbackIndex){
                    thumbScene.background = new THREE.Color("#CDA2E1")
                }
            }

            renderer.setScissorTest(true)
            renderer.setScissor((27-index) * size.x, 4, size.x, size.y);
            renderer.setViewport((27-index) * size.x, 4, size.x, size.y);
            if(starThumbTargets[index]){
                // starThumbScenes[index].background = new THREE.Color(Math.random(),Math.random(),Math.random())

                renderer.setRenderTarget((starThumbTargets[index]))
                renderer.render(starThumbScenes[index], thumbCamera);
                if (volsMaterial[index]) {
                    volsMaterial[index].uniforms["u_glyphWidth"].value = new THREE.Vector2(size.x, size.y)
                    volsMaterial[index].uniforms["u_glyphOffset"].value = new THREE.Vector2((27-index) * size.x, 4)

                    volsMaterial[index].uniforms["u_glyphDiffuse"].value = (starThumbTargets[index]).texture
                    volsMaterial[index].uniforms["u_glyphDepth"].value = (starThumbTargets[index]).depthTexture                    // starSaoMaterial.needsUpdate = true
                }
                renderer.setRenderTarget(null)
                // renderer.render(starThumbScenes[index], thumbCamera);
            }
            ...
            renderer.setScissor((27-index) * size.x, 4, size.x, size.y);
            renderer.setViewport((27-index) * size.x, 4, size.x, size.y);
            renderer.render(thumbScenes[index].thumbScene, thumbCamera);
            renderer.setScissorTest(false)
            thumbScene.background = new THREE.Color("rgba(14,16,123)")
            thumbScene.background = new THREE.Color("rgb(4,6,23)")
          });
    }

    // zoom-in render loop for temporal thumbnails
    if(thumbs == true && zoom_bool == true && temporalAnimation == true){
        let size = new THREE.Vector2(window.innerWidth / 28, window.innerWidth / 28);
        thumbZoomScenes.forEach(({ thumbZoomScene, thumbCamera }, index) => {
            if (temporalAnimation === true) {
                if(index === playbackIndex){
                    thumbZoomScene.background = new THREE.Color("#CDA2E1")
                }
            }
            renderer.setScissorTest(true)
            renderer.setScissor((27-index) * size.x, 8 + size.y, size.x, size.y);
            renderer.setViewport((27-index) * size.x, 8 + size.y, size.x, size.y);
            renderer.render(thumbZoomScene, thumbCamera);
            renderer.setScissorTest(false)
            // thumbScene.background = new THREE.Color("rgba(14,16,123)")
            thumbZoomScene.background = new THREE.Color("rgb(4,6,23)")
            });
...
function createColumnDensityInfoPanel(msg) {
    ...
    s.onchange = function () {
        ...
        if (s.value == 'metallicity') {
            ...
            c = 0
            for (let i = 0; i < msg.i_l.length; i++) {
...
function plotSyntheticSpectrum(points) {
    ...
    var margin = { top: 10, right: 30, bottom: 30, left: 30 },
        width = 300 - margin.left - margin.right,
        height = 200 - margin.top - margin.bottom;
    for (let i = 0; i < points.lambda.length; i++) {
...
function updateGraph() {
    ...
    if (skewerData.length) {
        for (let i = 0; i < skewerData.length; i++) {
            if (skewerData[i]) {
                let data = []
        ...
        for (const attr in galaxyBrushHistory) {
            ...
            if (galaxyBrushHistory[attr].checkState == true) {
                ...
                for (let i in filteredGalIds) {
                    ...
                    const dl = (filteredrh[i] / 1000) / (width_Mpc)  // half-width of virial halo in voxelized units

                    const geometry = new THREE.SphereGeometry(dl * gridsize, 32, 16);
                    const material = new THREE.MeshBasicMaterial({ color: 0xffff00 });
                    const sphere = new THREE.Mesh(geometry, material);

                    anchor.addEventListener('mouseover', (event) => {

                        console.log('hovering on this galaxy: ID',
                        ...
                        sphere.position.z = mz
                        skewerScene.add(sphere);

                    });

                    anchor.addEventListener('mouseout', (event) => {
                        skewerScene.remove(sphere)
                    });
                ...
                for (let i in filteredHaloIds) {
                    ...
                    anchor.addEventListener('click', (e) => {
                        ...
                        if (!singleSnapshot){
                            ...
                            zoomAnimationMode(haloSequences[filteredHaloIds[i]])
...
function updateXYZDomain(xyz, min, max) {
    controls.target.set(((domainXYZ[1] + domainXYZ[0]) * gridsize) / 2, ((domainXYZ[3] + domainXYZ[2]) * gridsize) / 2, ((domainXYZ[5] + domainXYZ[4]) * gridsize) / 2);
    controls.update()
    thumbCamera.position.copy(camera.position);
    thumbCamera.quaternion.copy(camera.quaternion);
    thumbCamera.updateProjectionMatrix()
...
async function checkSelectedSimID() {
    let selection = document.getElementById("sim_size_select")
    ...
    if (oldSimID != simID) {
        ...
        let starInfo = await loadStarGalaxyInfo()
...
function init() {
    ...
    thumbCamera = new THREE.PerspectiveCamera(45, 1, 0.01, 1000);
    thumbCamera.zoom = 1.45
    thumbCamera.updateProjectionMatrix();
